turtles-own [
  age             ;; the number of year this turtle has left
...
to turtle-setup ;; turtle procedure
  ...
  (ifelse
    ...
    [
      ...
      move-to one-of patches with [pxcor < 25 and pycor < 25 and not any? other turtles-here]
    ]
  )

  set age random-in-range 60 100
...
to go
  ...
  ask turtles [
    turtle-move
    turtle-eat
    if is-aging? ;; Using a switch as a global variable for Exercise 3.3
    [
      if age <= 0
      [
        hatch 1 [ turtle-setup ] ;; new turtle is born
        die                      ;; turtle dies of old age
      ]
      set age (age - 1) ;; turtle ages by one year
    ]
    if sugar <= 0
    [
        hatch 1 [ turtle-setup ] ;; new turtle is born
        die                      ;; turtle dies of old starvation
    ]
...
to turtle-share ;; turtle procedure
  let neighbour-turtles turtles-on patches in-radius vision
  let hungry-turtles neighbour-turtles with [sugar < [sugar] of myself]
  if any? hungry-turtles [
    let sherable-sugar min list (sugar * 0.3) (sugar - 10)
    if sherable-sugar > 0 [
      let sugar-gift sherable-sugar / count hungry-turtles
      ask hungry-turtles [
        set hungry-turtles  (sugar + sugar-gift)
      ]
      set sugar (sugar - sherable-sugar)
    ]
  ]
end
...
visualization
visualization
"no-visualization" "color-agents-by-vision" "color-agents-by-metabolism" "color-agents-by-sugar"
0
